...
function startCommand(context) {
  ...
  const userPassword = textParts.length > 1 ? textParts[1].trim() : null;
  
  const accessPassword = config.get("notifications.telegram.accessPassword");
  ...
  /stop`
      ...
      .then(() => {
        ...
        context.reply(`Not authenticated. Fuck you`);
...
async function loginCommand(context) {
  if(!isAuthenticated(context.chat.id)) { context.reply("You are not authenticated, motherfucker"); return; }
